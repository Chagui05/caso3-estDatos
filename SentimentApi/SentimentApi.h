#ifndef _SENTIMENTAPI_
#define _SENTIMENTAPI_ 1
#include <iostream>
#include <string>
#include <stdio.h>
#include <curl/curl.h>
...
public:
    ...
    SentimentInfo( string sentiment) : sentiment( sentiment) {}
};
...
public:
    SentimentInfo *getSentiment(string wordToAnalyze)
    {
        SentimentInfo *result = nullptr;
        ...
        // build the URL with the word
        string url = "https://api.api-ninjas.com/v1/sentiment?text=" + wordToAnalyze;
        ...
        {   
            ...
            result = new SentimentInfo(responseData["sentiment"].get<string>());
            free(req.buffer);
        }
        else
        {
    ...
    std::string getSentimentString(string wordToAnalyze){
        SentimentInfo *sentiment = getSentiment(wordToAnalyze);
